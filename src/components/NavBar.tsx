...
export default function NavBar({ onLogout }: Props) {
  ...
  return (
    <header className="border-b border-gray-200 bg-white px-3 sm:px-4 py-2 sm:py-3 flex justify-between items-center sticky top-0 z-50">
      <Link to="/dashboard" className="flex items-center gap-2 min-h-[44px]">
        <span className="text-xl">📚</span>
        <span className="text-lg font-bold text-sage-800 hidden sm:inline">Shareborough</span>
      </Link>
      <div className="flex items-center gap-1 sm:gap-2">
        <Link
          to="/dashboard"
          className="text-sm text-gray-600 hover:text-gray-900 px-2 sm:px-3 py-2 min-h-[44px] flex items-center rounded-lg hover:bg-gray-50 transition-colors"
        >
          My Libraries
        </Link>
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="flex items-center gap-1 px-1 py-1 rounded-lg hover:bg-gray-50 transition-colors min-h-[44px]"
          ...
          {menuOpen && (
            ...
            >
              ...
              <button
                ...
                className="block w-full text-left px-4 py-2 text-sm text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition-colors"
                role="menuitem"
              >
                Sign out
              </button>
