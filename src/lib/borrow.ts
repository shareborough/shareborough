...
export async function requestBorrow(params: RequestBorrowParams): Promise<BorrowRequest> {
  // Try RPC first (works when auth is not enabled or function is SECURITY DEFINER)
  try {
    ...
    // RPC failed (likely auth required) — fall back to direct CRUD
  }

  // Verify item is available first
